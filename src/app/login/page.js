...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Check if user is admin (manually added via UUID in DB)
      const { data: admin } = await supabase.from('admin').select('id').eq('id', userId).single();
      if (admin) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (data.user.identities && data.user.identities.length === 0) {
        throw new Error('Email sudah terdaftar. Gunakan tab "Masuk" untuk login.');
      }

      // Insert siswa record
      const { error: insertErr } = await supabase.from('siswa').insert({
        id: data.user.id,
      ...
      if (insertErr) {
        console.warn('Siswa insert at signup:', insertErr.message);
      }
  ...
  return (
    <div className="login-page">
      ...
      <div className="login-container animate-fadeIn">
        ...
        <div className="login-card">
          ...
          <div className="login-tabs">
            <button className={`login-tab ${!isRegister ? 'active' : ''}`} onClick={() => switchTab(false)}>Masuk</button>
            <button className={`login-tab ${isRegister ? 'active' : ''}`} onClick={() => switchTab(true)}>Daftar</button>
          </div>

          {/* Google Login Button — only for login tab */}
          {!isRegister && (
            <>
          ...
          <form onSubmit={isRegister ? handleRegister : handleLogin}>
            ...
            <div className="form-group">
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPw ? 'text' : 'password'} className="form-input" style={{ paddingRight: 44 }} placeholder="Minimal 6 karakter" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} />
                <button type="button" onClick={() => setShowPw(!showPw)} tabIndex={-1} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 4, display: 'flex' }}>
                  {showPw ? (
      ...
      <style jsx>{`
        ...
        .login-tab { flex: 1; padding: 10px; border-radius: 8px; border: none; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 200ms ease; background: transparent; color: var(--text-muted); font-family: inherit; }
        ...
        .google-btn {
          ...
          background: var(--bg-input); color: var(--text-primary); font-size: 14px; font-weight: 600;
          cursor: pointer; transition: all 200ms ease; font-family: inherit;
        }
        .google-btn:hover:not(:disabled) { background: var(--bg-surface-hover); border-color: var(--border-hover); box-shadow: 0 2px 8px rgba(0,0,0,0.2); }
